...
def integrate_pin_functions(main_window) -> bool: #vers 1
    """Integrate pin and lock functions into main window"""
    try:
        ...
        # Add lock functions to main window
        main_window.lock_selected_entries = lambda: lock_selected_entries(main_window)
        main_window.unlock_selected_entries = lambda: unlock_selected_entries(main_window)
        main_window.toggle_entry_lock_status = lambda entry_name: toggle_entry_lock_status(main_window, entry_name)
        main_window.get_locked_entries = lambda: get_locked_entries(main_window)
        main_window.is_entry_locked = lambda entry_name: is_entry_locked(main_window, entry_name)
        main_window.check_operation_allowed = lambda operation, entry_names=None: check_operation_allowed(main_window, operation, entry_names)
        ...
        main_window.log_message("✅ Pin and lock functions integrated")
        return True

    except Exception as e:
        main_window.log_message(f"❌ Failed to integrate pin/lock functions: {str(e)}")
        return False

def _update_table_pin_display(main_window, img_file) -> bool: #vers 1
    """Update table display to show pinned and locked entries"""
    try:
        ...
        table = main_window.gui_layout.table
        pinned_entries = getattr(img_file, '_pinned_entries', set())
        locked_entries = getattr(img_file, '_locked_entries', set())
        ...
        show_lock_icons = pin_options.get('show_lock_icons', True)
        ...
        # Apply highlighting and icons if enabled
        if highlight_pinned or show_lock_icons:
            _apply_pin_and_lock_styling(main_window, table, pinned_entries, locked_entries, show_lock_icons)
...
def _apply_pin_and_lock_styling(main_window, table, pinned_entries: Set[str], locked_entries: Set[str], show_lock_icons: bool) -> bool: #vers 1
    """Apply visual styling for pinned and locked entries in table"""
    try:
        from PyQt6.QtGui import QBrush, QColor, QFont
        from PyQt6.QtCore import Qt

        # Define colors
        pin_color = QColor(255, 255, 200)  # Light yellow for pinned
        lock_color = QColor(255, 230, 230)  # Light red for locked
        both_color = QColor(255, 240, 200)  # Light orange for both

        pin_brush = QBrush(pin_color)
        lock_brush = QBrush(lock_color)
        both_brush = QBrush(both_color)
        ...
        for row in range(table.rowCount()):
            ...
            if name_item:
                ...
                clean_name = entry_name.replace("📌 ", "").replace("🔒 ", "")

                is_pinned = clean_name in pinned_entries
                is_locked = clean_name in locked_entries

                # Build display name with icons
                display_name = clean_name
                if is_pinned and show_lock_icons:
                    display_name = f"📌 {display_name}"
                if is_locked and show_lock_icons:
                    if is_pinned:
                        display_name = f"🔒 {display_name}"  # Lock icon before pin
                    else:
                        display_name = f"🔒 {display_name}"

                # Apply background color
                if is_pinned and is_locked:
                    # Both pinned and locked
                    brush = both_brush
                elif is_locked:
                    # Locked only
                    brush = lock_brush
        ...
        main_window.log_message(f"⚠️ Failed to apply pin/lock styling: {str(e)}")
        return False


def _save_pin_state(main_window, img_file) -> bool: #vers 1
    """Save pin and lock state to file or memory"""
    try:
        if hasattr(img_file, '_pinned_entries') or hasattr(img_file, '_locked_entries'):
            # Create a simple serializable representation
            pin_data = {
                'pinned_entries': list(getattr(img_file, '_pinned_entries', set())),
                'locked_entries': list(getattr(img_file, '_locked_entries', set())),
                'pin_options': getattr(img_file, '_pin_options', {})
            }
        ...
        main_window.log_message(f"⚠️ Failed to save pin/lock state: {str(e)}")
        return False


def _load_pin_state(main_window, img_file) -> bool: #vers 1
    """Load pin and lock state from file or memory"""
    try:
        ...
        if hasattr(img_file, '_pin_state'):
            ...
            # Restore locked entries
            if 'locked_entries' in pin_data:
                img_file._locked_entries = set(pin_data['locked_entries'])
            ...
            # Initialize empty pin/lock state
            img_file._pinned_entries = set()
            img_file._locked_entries = set()
            img_file._pin_options = {
        ...
        main_window.log_message(f"⚠️ Failed to load pin/lock state: {str(e)}")
        return False


def get_pin_statistics(main_window) -> Dict[str, Any]: #vers 2
    """Get statistics about pinned and locked entries"""
    try:
        # Get current IMG file
        current_tab = main_window.tab_widget.currentWidget()
        if not current_tab or not hasattr(current_tab, 'img_file'):
            return {'total_entries': 0, 'pinned_entries': 0, 'locked_entries': 0}

        img_file = current_tab.img_file
        total_entries = len(getattr(img_file, 'entries', []))
        pinned_entries = len(getattr(img_file, '_pinned_entries', set()))
        locked_entries = len(getattr(img_file, '_locked_entries', set()))
        ...
        stats = {
            ...
            'lock_percentage': (locked_entries / total_entries * 100) if total_entries > 0 else 0,
            'pin_options': getattr(img_file, '_pin_options', {})
        }
        ...
        main_window.log_message(f"❌ Failed to get pin/lock statistics: {str(e)}")
        return {'total_entries': 0, 'pinned_entries': 0, 'locked_entries': 0}

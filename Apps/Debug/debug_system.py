...
"""
Debug System for Renderware Modding Suite
...
# Try to import colorama for colored output, fall back to no colors if not available
try:
    import colorama
    ...
    class Fore:
        LIGHTBLACK_EX = CYAN = GREEN = YELLOW = RED = MAGENTA = BLUE = ""
        LIGHTBLUE_EX = LIGHTGREEN_EX = LIGHTYELLOW_EX = LIGHTMAGENTA_EX = ""
        LIGHTCYAN_EX = WHITE = LIGHTRED_EX = ""
...
class DebugLogger:
    ...
    def __init__(self):
        ...
        # Color mapping for terminal output (only if colorama available)
    ...
    def _print_to_terminal(self, log_entry: dict):
        ...
        try:
            ...
            # Get colors (empty strings if colorama not available)
            level_color = self.level_colors.get(level, "")
            category_color = self.category_colors.get(category, "")
            white_color = Fore.WHITE if COLORAMA_AVAILABLE else ""
            gray_color = Fore.LIGHTBLACK_EX if COLORAMA_AVAILABLE else ""
            
            # Format main log line
            formatted_line = (
                f"{white_color}[{timestamp}] "
                f"{level_color}{level.name.ljust(8)} "
                f"{category_color}{category.value.ljust(12)} "
                f"{white_color}{message}"
            )
            
            # Add thread info if not main thread
            if thread != "MainThread":
                formatted_line += f" {gray_color}[{thread}]"
            ...
            if log_entry.get("data"):
                ...
                if isinstance(data, dict) and data:
                    try:
                        data_str = json.dumps(data, indent=6)[1:-1]  # Remove outer braces
                        print(f"{gray_color}    └─ Data: {data_str}")
                    except:
                        print(f"{gray_color}    └─ Data: {str(data)}")
            
            # Print source info for errors and critical logs
            if level.value >= LogLevel.ERROR.value and log_entry.get("source"):
                print(f"{gray_color}    └─ Source: {log_entry['source']}")
    ...
    def _print_performance_metric(self, perf_data: dict):
        ...
        if COLORAMA_AVAILABLE:
            ...
            print(f"[PERF] {operation} {duration_ms:.2f}ms")
